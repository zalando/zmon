...
if [ "b$1" = "b" ] || [ "b$1" = "beventlog-service" ] ; then
    ...
    docker run --restart "on-failure:10" --name zmon-eventlog-service --net host -e POSTGRESQL_USER=$PGUSER -e POSTGRESQL_PASSWORD=$PGPASSWORD -d os-registry.stups.zalan.do/stups/zmon-eventlog-service:$EVENTLOG_VERSION
...
if [ "b$1" = "b" ] || [ "b$1" = "bcontroller" ] ; then
    ...
    docker run --restart "on-failure:10" --name zmon-controller --net host -d os-registry.stups.zalan.do/stups/zmon-controller:$CONTROLLER_VERSION
